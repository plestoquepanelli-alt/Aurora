...
// ═══════════════════════════════════════════════════════════════
//  LOG DE ERROS — grava com timestamp em /aurora/logs/erros.txt
// ═══════════════════════════════════════════════════════════════
...
String gerarRelatorio(){
  ...
  // Gráfico: últimas 24 amostras
  int start = (n>24) ? n-24 : 0;
  int cols  = n - start;
  const int ROWS = 5;

  // ASCII chart temperatura
  String cT = "";
  ...
  String rel = "📊 *Relatório Semanal — Aurora*\n";
  rel += "━━━━━━━━━━━━━━━\n";
  ...
  // auto-limpeza: só apaga o CSV após a String estar montada com sucesso
...
void salvarContexto(String pergunta, String resposta){
  ...
  if(linhas >= MAX_PARES*2){
    ...
    if(!fc){ sdLogErro("SDCard","nao reescreveu contexto"); }
      ...
      fc.close();
      File fw=SD.open(ARQ,FILE_WRITE); if(fw){ fw.print(novo); fw.close(); }
    }
  }
  File f=SD.open(ARQ,FILE_APPEND); if(!f) return;
  f.println("P:"+pergunta); f.println("R:"+resposta); f.close();
}

String carregarContexto(){
  if(!sdOK) return "";
  File f=SD.open("/aurora/contexto/conversa.txt"); if(!f) return "";
  String c="";
...
String carregarPersonalidade(){
  File f=SD.open("/aurora/config/personalidade.txt"); if(!f) return "";
  String t="";
...
void salvarEvento(String data, String evento){
  File f=SD.open(arquivoAgendaMes(),FILE_APPEND); if(!f) return;
  f.println(data+";"+evento); f.close();
}

String lerAgenda(){
  File f=SD.open(arquivoAgendaMes()); if(!f) return "Agenda vazia.";
  String resp="📅 *Agenda do mês*\n\n";
  while(f.available()){
    String l=f.readStringUntil('\n');
    int sep=l.indexOf(';');
    if(sep>0){
...
void verificarLembretes(){
  ...
  if(f){
    while(f.available()){
      ...
      if(l.substring(0,sep).toInt()==dia){
        count++; String ev=l.substring(sep+1); ev.trim();
  ...
  String msg ="🌅 *Bom dia, Pedro!*\n━━━━━━━━━━━━━━━\n📅 *"+String(buf)+"*\n\n";
  msg += count>0 ? "📌 *"+String(count)+" evento(s) hoje:*\n"+eventos : "✅ Sem compromissos hoje.\n";
  msg += "\n⏱ Aurora online.";
...
String buscarMemoria(String pergunta){
  ...
  while(f.available()){
    String l=f.readStringUntil('\n'); int sep=l.indexOf(';'); if(sep<0) continue;
    String p=l.substring(0,sep), r=l.substring(sep+1);
    p.toLowerCase();
    if(pergunta.indexOf(p)>=0){ f.close(); return r; }
  }
  f.close(); return "";
}
...
void salvarMemoria(String pergunta, String resposta){
  ...
  f.print(pergunta); f.print(";"); f.println(resposta); f.close();
}
...
int contarPergunta(String pergunta){
  ...
  while(f.available()){
    ...
    p.toLowerCase();
    if(pergunta.indexOf(p)>=0){ f.close(); return c; }
  }
  f.close(); return 0;
}
...
void incrementarPergunta(String pergunta){
  ...
  String novo=""; bool enc=false;
  while(f.available()){
    String l=f.readStringUntil('\n'); int sep=l.indexOf(';');
    if(sep<0){ novo+=l+"\n"; continue; }
    String p=l.substring(0,sep); int c=l.substring(sep+1).toInt();
    String pL=p; pL.toLowerCase();
    if(pergunta.indexOf(pL)>=0){ c++; enc=true; novo+=p+";"+String(c)+"\n"; }
    else novo+=l+"\n";
  }
  f.close();
  if(!enc){
